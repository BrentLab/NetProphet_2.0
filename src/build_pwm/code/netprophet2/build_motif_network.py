...
def parse_args(argv):
    ...
    parser.add_argument('-r', '--fn_rids', dest='fn_rids', type=str, nargs='?')
    parser.add_argument('-g', '--fn_gids', dest='fn_gids', type=str, nargs='?')
    parser.add_argument('-n', '--p_in_net', dest='p_in_net', type=str)
...
def build_network(p_in_net, fn_inferred, dir_fimo, summary_suffix, thld_type, thld_val):
    from pandas import read_csv
    import numpy as np
    
    # initialize network
    df_in_net = read_csv(p_in_net, header=0, index_col=0, sep='\t')
#     rids = np.loadtxt(fn_rids, dtype=str)
#     gids = np.loadtxt(fn_gids, dtype=str)
    rids = np.array(list(df_in_net.index))
    gids = np.array(list(df_in_net.columns))
...
def build_subnetwork(motifs, gids, dir_fimo, summary_suffix):
    ...
    for j in range(len(motifs)):
        ...
        for k in range(len(gids)):
            gid = gids[k]
            adjmtr[j, k] = score_dict[gid] if gid in score_dict.keys() else 0
    return adjmtr
...
def write_adjmtr(adjmtr, p_in_net, fn):
    from pandas import DataFrame, read_csv
    
    # get the list of regulators and target genes
    df_in_net = read_csv(p_in_net, header=0, index_col=0, sep='\t')
    l_reg = list(df_in_net.index)
    l_target = list(df_in_net.columns)
    
    df_net = DataFrame(adjmtr)
    df_net.columns = l_target
    df_net.index = l_reg
    df_net.round(2).to_csv(fn, header=True, index=True, sep='\t')
#     writer = open(fn, "w")
...
def main(argv):
    ...
    network = build_network(parsed.p_in_net, parsed.fn_inferred, parsed.dir_fimo, parsed.summary_suffix, parsed.thld_type, parsed.thld_val)
    # write adjmtr file
    sys.stdout.write("DONE\nWriting network ... ")
    write_adjmtr(network, parsed.p_in_net, parsed.fn_adjmtr)
    sys.stdout.write("DONE\n")
