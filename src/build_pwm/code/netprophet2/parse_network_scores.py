...
def main(argv):
    ...
    df_net = read_csv(parsed.adjmtr, header=0, index_col=0, sep='\t')
    adjmtr = np.array(df_net)
    tfs = list(df_net.index)
    targets = list(df_net.columns)
...
#     targets = numpy.loadtxt(parsed.target, dtype=str, delimiter="\t")
    ...
    for i in range(1, adjmtr.shape[0], 1):
        writer = open(parsed.dir_output + tfs[i], "w")
        writer.write("#target\tscore\n")
        for j in range(1, adjmtr.shape[1], 1):
            if adjmtr[i, j] == 0:
                writer.write("%s\t0\n" % targets[j])
            else:
