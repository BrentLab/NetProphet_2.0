...
def main(argv):
    ...
    if (not is_np_net):
        ...
        df_np_component = read_csv(parsed.np_component
                                            , header=0
                                            , index_col=0
                                            , sep='\t')
        np_component = nmp.array(df_np_component)
#         np_component = nmp.loadtxt(parsed.np_component)
        ...
        binding_strengths = None
        if parsed.binding_strengths != None:
            binding_strengths = nmp.array(read_csv(parsed.binding_strengths
                                                   , header=0
                                                   , index_col=0
                                                   , sep='\t'))
#             binding_strengths = nmp.loadtxt(parsed.binding_strengths)
...
#     write_adjmtr(output_adjmtr_name, combined)
    ...
    DataFrame(combined
              , index=df_np_component.index
              , columns=df_np_component.columns).to_csv(
                    output_adjmtr_name
                    , header=True
                    , index=True
                    , sep='\t')
