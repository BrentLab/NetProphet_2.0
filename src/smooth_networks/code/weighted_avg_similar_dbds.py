...
def main(argv):
    ...
    df_network_input = read_csv(parsed.fn_adjmtr_network, header=0, index_col=0, sep='\t')
    network_input = np.array(df_network_input)
#     network_input = np.loadtxt(parsed.fn_adjmtr_network, delimiter='\t')[1:,1:] 
    sys.stdout.write("DONE\n")
    ...
    DataFrame(network_output
              , index=df_network_input.index
              , columns=df_network_input.columns).to_csv(
                                parsed.fn_output, header=True, index=True, sep='\t')
